...
`wireguard-tools`, loads the kernel module, and enables IPv4/IPv6 forwarding (persisted via
`/etc/sysctl.d/99-rebecca-wireguard.conf`). Each WireGuard inbound owns its own kernel interface,
UDP listen port, address pool and NAT rules, so multiple inbounds coexist on distinct ports. The
master pushes WireGuard state inside the shared runtime envelope (`ov_runtime_json`) under
`wg_inbounds`, each with a server `private_key`, `address_pool`, `listen_port` and a `peers` list
(`public_key`, optional `preshared_key`, `address`). Per-peer rx+tx is read from the kernel and
reported to the master as `wg:<user_id>` usage deltas.
